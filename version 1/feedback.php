...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_feedback'])) {
    ...
    if (!empty($feedback_msg)) {
        // Ensure user_id column exists dynamically in case the schema wasn't migrated
        $conn->query("ALTER TABLE feedback ADD COLUMN IF NOT EXISTS user_id INT DEFAULT NULL");

        // Reference the logged-in user's ID and name in the 'name' field of our schema
        $feedback_author = "User ID: " . $user_id . " (" . $username . ")";
        
        // VULNERABLE INSERT: Direct concatenation of user input (Stored XSS source)
...
// Fetch all feedback submissions to display (demonstrating Stored XSS sink)
            ...
            <div class="row">
                ...
                <div class="col-md-9 col-lg-10 py-4 px-4 bg-light">
                    ...
                    <div class="row">
                        ...
                        <!-- Right Panel: Submitted Feedbacks (demonstrating Stored XSS sink) -->
                                ...
                                <?php if ($result_feedback && $result_feedback->num_rows > 0): ?>
                                    <?php while ($row = $result_feedback->fetch_assoc()): ?>
                                        <div class="card feedback-item mb-3 shadow-sm p-3">
                                            ...
                                            <!-- VULNERABLE RENDER: No htmlspecialchars() validation (Stored XSS sink) -->
